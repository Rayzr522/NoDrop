...
public class NoDrop extends JavaPlugin implements Listener {
    ...
    public void onItemDrop(PlayerDropItemEvent e) {
        ...
        if (e.getPlayer().hasPermission(config.PERM_EXEMPT)) {
            return;
        }
        if (!config.worlds.contains(e.getPlayer().getWorld().getName())) {
            return;
        }
        e.setCancelled(true);
    }
    ...
    public void onInventoryClick(InventoryClickEvent e) {
        if (!config.PREVENT_CLICK) {
            return;
        }
        if (e.getWhoClicked().hasPermission(config.PERM_EXEMPT)) {
            return;
        }
        if (!config.worlds.contains(e.getWhoClicked().getWorld().getName())) {
            return;
        }
        e.setCancelled(true);
    }
    ...
    public void onDeath(PlayerDeathEvent e) {
        if (!config.PREVENT_DEATH) {
            return;
        }
        if (e.getEntity().hasPermission(config.PERM_EXEMPT)) {
            return;
        }
        if (!config.worlds.contains(e.getEntity().getWorld().getName())) {
            return;
        }
        e.getDrops().clear();
    }
